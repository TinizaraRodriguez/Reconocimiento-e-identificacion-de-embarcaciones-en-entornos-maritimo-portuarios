...
import numpy as np
from directory_utils import checkDirectory, searchImages, resize_image
from metrics import execTime, gpuMemUse, calculate_metrics
from read_json_file import readFiles, obtain_text_region

# funcion de inicialziacion del tesseract
def tesseract(args, start_time):
    ...
    if checkDirectory(args.inputdirectory) & checkDirectory(output_dir, True):
        ...
        model_folder = os.path.join(args.outputdirectory, "TesseractOCR")
        ...
        for img_path in image_files:
            process_image(img_path, args, error_folder, results_folder)
            end_time = time.time()
...
def process_image(img_path, args, error_folder, results_folder):
    try:
        ...
        if text_region_coordinates:
            ...
            for i, text_coord in enumerate(text_region_coordinates, 1):
                ...
                detected_texts = perform_ocr(gray_scale_image, (x1, y1), (x2, y2), created_folders[4])

                # Escribir el texto detectado encima del rectángulo
                if detected_texts:
                    for text in detected_texts:
                        cv2.putText(original_np, f"{text}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
def perform_ocr(image, text_coord_start, text_coord_end, directory):
    detected_text = []
