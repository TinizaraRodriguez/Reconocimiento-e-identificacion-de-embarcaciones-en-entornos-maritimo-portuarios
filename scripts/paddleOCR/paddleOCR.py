...
# funcion de inicialziacion del paddleOCR
def paddleOcr(args, start_time):
    # Configuración de PaddleOCR
    ocr = PaddleOCR(lang='en', use_gpu=True, cls=True, use_angle_cls=True, show_log=False)
    print("Inicio de paddle")
    ...
    if checkDirectory(args.inputdirectory) & checkDirectory(output_dir, True):
        ...
        model_folder = os.path.join(args.outputdirectory, "PaddleOCR")
    ...
    for line in result:
        if line is not None:
            for word_info in line:
                ...
                if (text, confidence) not in detected_texts:
