...
# funcion de inicialziacion del kerasOCR
def kerasOCR(args, start_time):
    ocr = keras_ocr.pipeline.Pipeline()
    print("Inicio de kerasOCR")
    ...
    if checkDirectory(args.inputdirectory) & checkDirectory(output_dir, True):
        ...
        model_folder = os.path.join(args.outputdirectory, "kerasOCR")
        ...
        for img_path in image_files:
            start_gpu_mem = gpuMemUse(args.device.split(":")[1])

            process_image(img_path, ocr, args, error_folder, results_folder)
            
            end_time = time.time()
            end_gpu_mem = gpuMemUse(args.device.split(":")[1])
            
            if start_gpu_mem is not None and end_gpu_mem is not None:
                gpu_mem = end_gpu_mem - start_gpu_mem
                gpu_mem_use.append(gpu_mem)
    ...
    try:
        ...
        if text_region_coordinates:
            ...
            for i, text_coord in enumerate(text_region_coordinates, 1):
                ...
                text_region_np = original.crop((x1,y1,x2+20,y2+20))
                text_region_np.save(crop_text_path)
                ...
                #llamamos a la funcion que procesara los archivos de texto
...
def perform_ocr(image, ocr, text_coord_start, text_coord_end, directory):
    ...
    with open(output_file, "w") as f:
        for text in predictions[0]:
            ...
            if detected_text not in detected_texts:
                with open(output_file, "a+") as f:
                    f.write(f"Texto detectado: {detected_text}\n")
                    f.write(f"Coordenadas: [[{text_coord_start[0]}, {text_coord_start[1]}], [{text_coord_end[0]}, {text_coord_end[1]}]]\n")
                    f.write("\n")
            detected_texts.add(detected_text)
